...
from typing_extensions import AsyncIterator, Required, TypedDict
...
async def main() -> None:
    ...
    endpoint = (
        ...
        or "https://your-endpoint.azure.com/"
    )
    ...
    try:
        ...
        while True:
            if stream.read_available >= read_size:
                ...
                logger.debug(f"Read {len(data)} frames from audio stream")
                ...
                logger.debug(f"Sending audio data of size {len(data_json)} bytes")
        ...
        while True:
            async for raw_event in connection:
                ...
                if event.get("type") == "session.created":
                    ...
                    raise ValueError(
                        f"Error received: Type={error_type}, Code={error_code}, Message={error_message}"
                    )
